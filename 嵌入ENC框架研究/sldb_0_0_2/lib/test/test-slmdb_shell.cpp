...
int main(void)
{
    ...
    slmdb_shell slmdb_shell_;
    slmdb_shell_.open(6, "./slmdb.db");

    // insert =============================================================================
    std::cout << "Test insert ..." << std::endl;

    std::cout << "-- before insert" << std::endl;
    std::cout << "-- size: " << slmdb_shell_.size() << "\n" << std::endl;
    ...
    for (register int i = 0; i < T; i ++)
    {
        ...
        // len = strlen + 1
        slmdb_shell_.insert(key_fmt.str().c_str(), key_fmt.str().length() + 1, val_fmt.str().c_str(), val_fmt.str().length() + 1);
    }
    ...
    for (register int i = T; i < 2 * T; i ++)
    {
        ...
        // len = memlen
        slmdb_shell_.insert(&key_buf, sizeof(int), &val_buf, sizeof(time_t));
    }

    std::cout << "-- after insert" << std::endl;
    std::cout << "-- size: " << slmdb_shell_.size() << std::endl;
    std::cout << "\n" << std::endl;
    ...
    for (register int i = 0; i < T; i ++)
    {
        ...
        // len = strlen + 1
        if (slmdb_shell_.fetch(key_fmt.str().c_str(), key_fmt.str().length() + 1, &val_buf, &val_len) && (val_len != 0))
        {
    ...
    for (register int i = T; i < 2 * T; i ++)
    {
        ...
        // len = memlen
        if (slmdb_shell_.fetch(&key_buf, key_len, &val_buf, &val_len) && (val_len != 0))
        {
    ...
    if (slmdb_shell_.first(&key_sptr, &key_size))
    {
        register int i = 0;
        nkey_sptr = key_sptr;
        nkey_size = key_size;
        do
        {
            key_sptr = nkey_sptr;
            key_size = nkey_size;
            slmdb_shell_.fetch((void*)(key_sptr.get()), key_size, &val_sptr, &val_size);
            char* tmp = reinterpret_cast<char*>(val_sptr.get());
            std::cout << ++ i << "\t:" << key_sptr.get() << "\t=> " << tmp << std::endl;
            slmdb_shell_.mark_erasure(key_sptr, key_size);
        } while (slmdb_shell_.next(key_sptr.get(), key_size, &nkey_sptr, &nkey_size) && (i < T));
        slmdb_shell_.erase_marked();
    }
    ...
    if (slmdb_shell_.first(&key_sptr, &key_size))
    {
        ...
        do
        {
            ...
            slmdb_shell_.mark_erasure(key_sptr, key_size);
        } while (slmdb_shell_.next(key_sptr.get(), key_size, &nkey_sptr, &nkey_size));
        slmdb_shell_.erase_marked();
    }
