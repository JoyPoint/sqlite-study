...
// construction =======================================================================
slfdb_shell::slfdb_shell(void)
    : _sldb(NULL),          // Êý¾Ý¿â¾ä±ú
      _stmt_insert( NULL), 
      _stmt_replace(NULL), 
      _stmt_fetch(  NULL), 
      _stmt_erase(  NULL), 
      ...
      _stmt_next(   NULL), 
      _stmt_size(   NULL),
      _stmt_tbl_exist(NULL), // statement
      _path("./slfdb.db")   // Â·¾¶
{
...
// public =============================================================================
bool 
slfdb_shell::open(const std::string& path_)
{
    int rc = sqlite3_open(path_.c_str(), &_sldb);
    if (SQLITE_OK == rc)
    {
        //std::cout << "[success] open database, _sldb: " << _sldb << std::endl;
        // sync optimize ......
        optimize(_sldb, FDB);
        if (! tbl_exist())
        {
...
void 
slfdb_shell::close(void)
{
    if (_stmt_insert != NULL)
    {
        //std::cout << "_sldb: " << _sldb << "\t_stmt_insert : " << _stmt_insert << std::endl;
        sqlite3_finalize(_stmt_insert);
        _stmt_insert = NULL;
    }
    if (_stmt_replace != NULL)
    {
        //std::cout << "_sldb: " << _sldb << "\t_stmt_replace: " << _stmt_replace << std::endl;
        sqlite3_finalize(_stmt_replace);
        _stmt_replace = NULL;
    }
    if (_stmt_fetch != NULL)
    {
        //std::cout << "_sldb: " << _sldb << "\t_stmt_fetch  : " << _stmt_fetch << std::endl;
        sqlite3_finalize(_stmt_fetch);
        _stmt_fetch = NULL;
    }
    if (_stmt_erase != NULL)
    {
        //std::cout << "_sldb: " << _sldb << "\t_stmt_erase  : " << _stmt_erase << std::endl;
        sqlite3_finalize(_stmt_erase);
        _stmt_erase = NULL;
    }
    if (_stmt_exist != NULL)
    {
        //std::cout << "_sldb: " << _sldb << "\t_stmt_exist  : " << _stmt_exist << std::endl;
        sqlite3_finalize(_stmt_exist);
        _stmt_exist = NULL;
    }
    if (_stmt_first != NULL)
    {
        //std::cout << "_sldb: " << _sldb << "\t_stmt_first  : " << _stmt_first << std::endl;
        sqlite3_finalize(_stmt_first);
        _stmt_first = NULL;
    }
    if (_stmt_next != NULL)
    {
        //std::cout << "_sldb: " << _sldb << "\t_stmt_next   : " << _stmt_next << std::endl;
        sqlite3_finalize(_stmt_next);
        _stmt_next = NULL;
    }
    if (_stmt_size != NULL)
    {
        //std::cout << "_sldb: " << _sldb << "\t_stmt_size   : " << _stmt_size << std::endl;
        sqlite3_finalize(_stmt_size);
        _stmt_size = NULL;
    }
    if (_stmt_tbl_exist != NULL)
    {
        //std::cout << "_sldb: " << _sldb << "\t_stmt_tbl_exist   : " << _stmt_tbl_exist << std::endl;
        sqlite3_finalize(_stmt_tbl_exist);
        _stmt_tbl_exist = NULL;
    }
...
void 
slfdb_shell::sync(void)
{
    dump();
}

bool 
slfdb_shell::insert(const void* key_buf, size_t key_len, const void* val_buf, size_t val_len)
{
    int rc = 0;
...
bool 
slfdb_shell::replace(const void* key_buf, size_t key_len, const void* val_buf, size_t val_len)
{
    int rc = 0;
...
bool 
slfdb_shell::fetch(const void* key_buf, size_t key_len, char_sptr* val_buf, size_t* val_len)
{
    if (_stmt_fetch != NULL)
...
bool 
slfdb_shell::erase(const void* key_buf, size_t key_len)
{
    int rc = 0;
    if (_stmt_erase != NULL)
...
bool 
slfdb_shell::exist(const void* key_buf, size_t key_len)
{
    int rc = 0;
    if (_stmt_exist != NULL)
...
bool 
slfdb_shell::first(char_sptr* key_buf, size_t* key_len)
{
    //if (NULL == _stmt_first)
    //{
...
bool 
slfdb_shell::next(const void* key_buf, size_t key_len, char_sptr* nkey_buf, size_t* nkey_len)
{
    if (_stmt_next != NULL)
    ...
    int nextid = get_id_from_key(key_buf, key_len);
...
/*
size_t
slfdb_shell::mark_erasure(const char_sptr& key_buf, size_t key_len)
{
    erase(key_buf.get(), key_len);
    return 0;
}
size_t
slfdb_shell::erase_marked(void)
{
    return 0;
}
*/
...
slfdb_shell::erase_marked(void)
{
    ...
    info % path() % res_size % max_size % size();
...
slfdb_shell::size(void)
{
    ...
    return (size_t)sqlite3_column_int(_stmt_size, 0);
}

// private ============================================================================
bool 
slfdb_shell::dump(void)
{
    int rc = 0;
...
bool 
slfdb_shell::tbl_exist(void)
{
    int rc = 0;
    if (NULL == _stmt_tbl_exist)
    {
        rc = sqlite3_prepare(_sldb, _sql_tbl_exist, -1, &_stmt_tbl_exist, 0);
        if (rc != SQLITE_OK)
        {
    ...
    rc = sqlite3_step(_stmt_tbl_exist);
    if (rc != SQLITE_ROW)
    {
        std::cout << "[error] <tbl_exist> sql step" << std::endl;
        return false;
    }
    int flag = sqlite3_column_int(_stmt_tbl_exist, 0);
...
int 
slfdb_shell::get_id_from_key(const void* key_buf, size_t key_len)
{
...
int 
slfdb_shell::get_id_from_val(const void* val_buf, size_t val_len)
{
    return 0;
}

int 
slfdb_shell::get_max_id(void)
{
    sqlite3_stmt* stmt = NULL;
